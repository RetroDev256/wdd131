...
function recipeTemplate(recipe) {
    let template = `
        <div class="food">
            <img class="food_image" src="${recipe.image}" alt="">
            <div class="food_information">
                <div class="food_label_list">
    `;

    recipe.tags.forEach((tag) => {
        template += `<p class="food_label">${tag}</p>`;
    });

    template += `
                </div>
                <p class="food_name">${recipe.name}</p>
                <p class="food_rating">${stars(recipe.rating)}</p>
                <p class="food_description">${recipe.description}</p>
            </div>
        </div>
    `;
